...
// Función para generar una pregunta aleatoria sobre un país
async function generateQuestion(country) {
  const questionType = Math.floor(Math.random() * 3); // 0, 1, o 2

  switch (questionType) {
    case 0: // Pregunta sobre la capital del país
      return {
        type: 0,
    ...
    case 1: // Pregunta sobre la bandera del país
      return {
        type: 1,
    ...
    case 2: // Pregunta sobre los países limítrofes
      return {
        type: 2,
...
// API para comenzar un nuevo juego
app.post("/api/start", async (req, res) => {
  try {
    ...
    // Almacenar la información del jugador
    users[userId] = {
      name: playerName,
      score: 0,
      correctAnswers: 0,
    ...
    // Verificar que haya suficientes preguntas generadas
    if (questions.length < 5) {
      throw new Error("No se pudieron generar suficientes preguntas");
    }

    // Guardar las preguntas en la sesión del jugador
...
// Endpoint para guardar una partida
